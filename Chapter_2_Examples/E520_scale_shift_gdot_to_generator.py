...
# Create a group element at which to construct a velocity vector
g = G.element([1.5, 1])

# Create a velocity at that point
g_dot = G.vector(g, [1, 1])


# Create vectors on the respective generator fields
# d_dL = g_dot.L_generator(grid_points)
# d_dR = g_dot.R_generator(grid_points)

print(g_dot.L_generator(g))
print(g_dot.R_generator(g))
