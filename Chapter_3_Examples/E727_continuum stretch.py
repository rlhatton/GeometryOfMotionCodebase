...
def stretch_continuum(shape_params, s, t):
    epsilon = shape_params[0]
    h_circ_a = G.Lie_alg_vector([-epsilon* np.cos(s*4*np.pi), 0, 0])
    return h_circ_a


# Form the links, joints, and grounding point into a chain
chain = cb.ContinuumBody(stretch_continuum, [0, 1], gp)
