...
arc_axis = G.Lie_alg_vector([1, 0, 1])
link_transform = G.element([1, 0, 0])
arc_transform = G.Lie_alg_vector([1, 0, 1]).exp_R
...
# Then place an arc joint and an arc piston link whose base extension is the midpoint of the link
joints.append(kc.Joint(arc_axis, kc.arc_joint(1, .05, .25)))
links.append(kc.Link(arc_transform, kc.arc_link(1, 1, .5, .025)))
