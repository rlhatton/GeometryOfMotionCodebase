...
class ManifoldFunction:
    ...
    def __call__(self, *call_args, **kwargs):
        ...
        configuration_grids, process_args, value_type = self.preprocess(*call_args)

        # Evaluate
        function_grids = self.process(configuration_grids, *process_args, **kwargs)

        # Apply any post-process formatting
        function_value = self.postprocess(configuration_grids, function_grids, value_type)

        return function_value

    def preprocess(self, *call_args):
        """Configuration_value input is first element of call_args, and can be one of:
                    1. ManifoldElement
                    2. ndarray or List of int/float values describing a configuration element
                    3. ManifoldElementSet
                    4. GridArray of values describing configuration points. Assumed to be component-outer (as generated by
                        meshgrid_array), can be changed by setting the 'input_grid_format' parameter on the ManifoldFunction
                        from 'component' to 'element' after creation."""

        configuration_value = call_args[0]
        process_args = call_args[1:]
        ...
        if (isinstance(configuration_value, ManifoldElement) or
                ...
                or (isinstance(configuration_value, list)
                    and (ut.shape(configuration_value) == self.manifold.element_shape)
                    and isinstance(configuration_value[1], (float, int)))):
            ...
            if isinstance(configuration_value, ManifoldElementSet):
                configuration_grid = configuration_value.grid
            else:
                configuration_grid = configuration_value

            # Format the grid to be element-first
            configuration_grid = ut.format_grid(configuration_grid, self.manifold.element_shape, 'element')
        ...
        return configuration_grid, process_args, value_type

    def process(self, configuration_grid, *process_args, **kwargs):
